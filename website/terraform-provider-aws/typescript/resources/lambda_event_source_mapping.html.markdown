...
* `uri` - (Required) The URI for this configuration.  For type `VPC_SUBNET` the value should be `subnet:subnetId` where `subnetId` is the value you would find in an aws\_subnet resource's id attribute.  For type `VPC_SECURITY_GROUP` the value should be `securityGroup:securityGroupId` where `securityGroupId` is the value you would find in an aws\_security\_group resource's id attribute.
